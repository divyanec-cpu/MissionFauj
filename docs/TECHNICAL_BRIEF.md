...
- **Sign-out is a plain logout that deletes nothing** — `AppStateContext.signOut` clears only `auth`. It can afford to, because storage is scoped per candidate (see §4): the same person signing back in finds their own bucket untouched, and a different person signing in gets an empty one. Deleting on sign-out was considered and rejected — it "fixes" the shared-device leak by destroying the departing candidate's own progress, and since none of this exists server-side, that loss is unrecoverable. Scoping solves both halves without that cost.
- **`RootGate` (`App.tsx`) gates on `candidateName` alone**, not `candidateName && candidatePath` — both are actually set together (`completeSetup`), but checking name alone is sufficient since path is always chosen first in the same linear flow, and it avoids a real bug that surfaced during development: `OnboardingPage`'s last step never rendered when `completeSetup` was called immediately after the Name step, because writing `candidateName` to context synchronously flips `RootGate` from `OnboardingPage` to `HomePage`, unmounting the whole onboarding component — including its own local `step` state — before the later steps ever got a chance to show. Fixed by keeping name/path (and now also the profile draft + computed eligibility results) in `OnboardingPage`'s local state through every step, calling `appState.completeSetup(name, path)` **and** `appState.setProfileAndEligibility(profile, results)` together only at the point the candidate clicks through the final report screen — not at the Name or Profile step. Any future onboarding step added after the one that "completes" a gate needs the same care — don't write the gating field to context until the flow is actually done.

## 4. Data Model

**Storage scoping.** `auth` is stored unscoped at `missionfauj:auth` — it records *who* is signed in and selects the scope. Every other field belongs to one candidate and is stored under `missionfauj:u:<phone>:<field>`, so a device shared by two candidates keeps two separate buckets rather than one shared one. This is what lets sign-out delete nothing (§3). `usePersistedState` therefore has to tolerate its key changing at runtime: it re-reads during render (not in an effect) when the key changes, otherwise the persist effect would write the previous candidate's value under the new candidate's key — verified by checking that signing out leaves `anon:*` holding defaults rather than the departing candidate's values. A one-time `migrateUnscopedKeys` moves data written by pre-scoping builds into the signed-in candidate's bucket and deletes the originals; if nobody is signed in at upgrade time that data can't be attributed to a phone, so it is left in place unread rather than handed to whoever signs in next.

Note this **scopes** data per candidate, it does not **secure** it — anything in `localStorage` is readable by anyone who can open devtools on the device. Real isolation requires the data to live server-side behind the session JWT, which is also what's needed before subscriptions can survive a lost or reinstalled device (§3).

**Local only** (`localStorage`, via `usePersistedState`, key-namespaced `missionfauj:*`):
`auth` (`VerifiedAuth`: candidatePhone, age, isMinor, guardianName?, guardianPhone?, consentAcceptedAt), `candidateName` (set once during onboarding — see §3), `candidatePath` (`CandidatePath`: `'school'|'graduate'|'ssb-only'` — drives which Written Exam Prep tabs show, `src/types/candidatePath.ts`), `profile` (`CandidateProfile`: age, gender, marital, education, stream, ncc — collected as a mandatory onboarding step so it's always real, never a default; see `ProfileDraft`/`isProfileComplete` in `src/types/profile.ts` for how "every field actually answered" is enforced before submit is even clickable), `eligibilityResults` (`SchemeResult[]`, computed from `profile` in the same onboarding flow), `writtenSubscriptions` (per-exam `SubscriptionState`), `ssbSubscription`, `ssbRegistration`, `aiUsage` (per-surface free-question counters). Accounts created before this profile-at-onboarding flow shipped may still have `profile`/`eligibilityResults` as `null` — `HomePage`'s nudge card and the `/eligibility-check` page (§5) are the backfill path for those, using the same shared `ProfileStep` component.
...
- **The eligibility check never gates prep content**, even though collecting the profile it needs is now mandatory at onboarding. `WrittenExamPrepPage` decides which exam tabs to show purely from `candidatePath` (school/graduate/ssb-only) — it has no dependency on `eligibilityResults` at all. Rationale: a pass/fail eligibility check is not a legitimate reason to block someone from studying — the app's job is to help candidates prepare, not to pre-judge who's "allowed" to. `/eligibility-check` (Functional Spec §5) remains reachable any time from the header for retaking the scan later.
- **No pre-selected defaults on any profile field.** `ProfileStep` (`src/components/ProfileStep.tsx`, shared by onboarding and the standalone Eligibility Check retake) renders every option unselected until the candidate picks one, and its submit button stays disabled until `isProfileComplete` is true. This replaced an earlier version that initialized the form from a `DEFAULT_PROFILE` constant (Male/Unmarried/Class 12 (appearing)/Science (PCM)/NCC None) — candidates who clicked through without reviewing those pre-highlighted pills got an eligibility report computed from data they never actually entered. Don't reintroduce a prefilled default for any field but `age` (which is real, verified data from the Login Sequence, not a guess).
...
- **`/admin/diagnostics`** (`server/src/routes/admin/diagnostics.ts`): connectivity checks that can only be answered from the deployed server, not a dev machine. Currently one probe — whether this server can reach MSG91's widget API — which decides whether OTP verification can be made server-authoritative (§7). It posts a deliberately fake authkey, so no credential is exposed, and reads the verdict off the *shape* of the reply rather than the status code: MSG91 answers `{"type":"error","code":"201"}` with **HTTP 200** for bad credentials, so a JSON body means reachable and an HTML page or timeout means the anti-abuse layer is intercepting. Behind the admin session regardless, since an endpoint that makes the server call out on request shouldn't be left open.

## 7. Known Issues / Open Items

- **Candidate OTP verification is not server-authoritative, and there is no candidate session at all.** `/auth/otp-sent` accepts any `{phone, purpose, reqId}` unauthenticated, and `/auth/verify-otp` only checks that a fresh bookkeeping row exists rather than confirming the code with MSG91 — so two unauthenticated POSTs mint a token asserting ownership of any phone number, with no SMS involved. The blast radius today is small and bounded: those tokens live 20 minutes, are never persisted client-side (`VerifiedAuth` carries no token), and the only thing they unlock is writing a `ConsentRecord` — **no candidate data is readable over the API**. That is the entire reason it has been acceptable. It stops being acceptable the moment any per-candidate data moves server-side: the same forgery would then read and write any candidate's profile (much of it belonging to minors) and grant subscriptions, remotely and at scale — strictly worse than today's device-local storage, which at least requires physical possession. **So server-side candidate state is blocked on fixing this first**, most likely by having the client forward MSG91's access token from `verifyOtp` (currently discarded in `msg91Client.ts`) for the server to validate via `verifyAccessToken` with a server-only authkey. Whether that is viable depends on the IP-blocking item below, which is what `/admin/diagnostics` exists to answer.
- **The local backend runs on port 4010, not 4000**, because another project on the same dev machine (CoachYaar) defaults to 4000 and its file-watcher re-claims the port whenever it restarts. This matters because the collision fails *misleadingly* rather than loudly: both apps expose `/health`, so the wrong server answers it happily while every MissionFauj route returns 404 — which reads exactly like a stale build or a broken router mount. It cost two wrong diagnoses ("stale process") before the actual cause was found by tracing the listening PID's parent. If MissionFauj routes 404 locally while `/health` is fine, check *what* is listening (`netstat -ano | grep :4010`, then trace the PID's parent) before touching the code.
...
# Backend dev
cd server && npm install && npm run dev         # http://localhost:4010 (not 4000 — see below)
